  ...
  useEffect(() => {
    ...
    if (discoverStart && username && !localStorage.getItem("virtualId")) {
      (async () => {
        try {
          ...
          const usernameDetails = await axios.post(
            `${process.env.REACT_APP_VIRTUAL_ID_HOST}/${config.URLS.GET_VIRTUAL_ID}?usernamea=${username}`
          );
      ...
      (async () => {
        ...
        try {
          const getMilestoneDetails = await axios.get(
            `${process.env.REACT_APP_LEARNER_AI_APP_HOST}/${config.URLS.GET_MILESTONE}/${virtualId}?langudage=${language}`
          );
          localStorage.setItem(
            "getMilestone",
            JSON.stringify({ ...getMilestoneDetails.data })
