...
export const storeModel = async (modelName, modelURL, setDownloadProgress, isVocabModel) => {
  try {
...
export const isModelStored = (modelName) => {
  return new Promise((resolve, reject) => {
    ...
    request.onerror = (event) => {
      console.error("Error checking model in IndexedDB:", event.target.errorCode);
      reject(event.target.error);
    };
  ...
  try {
    ...
    request.onsuccess = async () => {
      ...
      if (window.offlineSession === undefined) {
        window.offlineSession = await window.ort.InferenceSession.create(modelData);
...
export const handleRedirect = async (lang, callback, setDownloadProgress) => {
  if (localStorage.getItem("isOfflineModel") === "true") {
    ...
    isModelStored(modelName)
      .then((stored) => {
        if (!stored) {
          console.log(`Model ${modelName} is not stored, starting download in background.`);
