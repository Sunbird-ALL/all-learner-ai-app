const kn = {
  // Navigation
  NEXT: "ಮುಂದೆ",

  // Actions
  SPEAK: "ಮಾತನಾಡಿ",
  LISTEN: "ಕೇಳಿ",

  // Status
  LOADING_GAME: "ಆಟ ಲೋಡ್ ಆಗುತ್ತಿದೆ...",

  // Feedback
  HURRAY: "ಅಭಿನಂದನೆಗಳು!!!",
  ALMOST_THERE: "ಬಹುತೇಕ!!!",
  DEMO_COMPLETE: "ಡೆಮೊ ಪೂರ್ಣಗೊಂಡಿದೆ!",
  DEMO_COMPLETION_SUBTITLE: "ನೀವು ಮಾತನಾಡಲು ಪ್ರಾರಂಭಿಸಲು ಸಿದ್ಧರಾಗಿದ್ದೀರಿ!",
  DEMO_COMPLETION_MESSAGE:
    "ಅದ್ಭುತ! ನಿಮ್ಮ ಧ್ವನಿಯನ್ನು ರೆಕಾರ್ಡ್ ಮಾಡುವುದು ಮತ್ತು ಕೇಳುವುದು ಹೇಗೆ ಎಂದು ನೀವು ಕಲಿತಿದ್ದೀರಿ. ಈಗ ನೈಜ ವಾಕ್ಯಗಳೊಂದಿಗೆ ಅಭ್ಯಾಸ ಮಾಡುವ ಸಮಯ!",
  DEMO_COMPLETION_START_GAME: "ಮಾತನಾಡಲು ಪ್ರಾರಂಭಿಸಿ",
  DEMO_COMPLETION_REPLAY: "ಡೆಮೊವನ್ನು ಮರು ಪ್ಲೇ ಮಾಡಿ",

  // Errors
  GAME_ERROR: "ಆಟವನ್ನು ಲೋಡ್ ಮಾಡುವಲ್ಲಿ ದೋಷ",
  NO_OPTIONS: "ಯಾವುದೇ ಆಯ್ಕೆಗಳು ಲಭ್ಯವಿಲ್ಲ",

  // Microphone / Audio

  // Phone conversation activity
  CONVO_DONE: "ಸಂಭಾಷಣೆ ಪೂರ್ಣಗೊಂಡಿದೆ!",

  // Session/navigation

  // Common actions (new keys)
  COMMON_BACK: "ಹಿಂದೆ",
  COMMON_CONTINUE: "ಮುಂದುವರಿಸಿ",

  // Errors (extended)

  // Language selection

  // Practice — activity headers
  PRACTICE_GUESS_IMAGE: "ಕೆಳಗಿನ ಚಿತ್ರವನ್ನು ಊಹಿಸಿ",
  PRACTICE_SPEAK_WORD: "ಕೆಳಗಿನ ಪದವನ್ನು ಹೇಳಿರಿ",
  PRACTICE_SPEAK_BELOW: "ಕೆಳಗೆ ಇರುವುದನ್ನು ಹೇಳಿರಿ",
  PRACTICE_READ_QUESTION_RECORD_RESPONSE:
    "ಪ್ರಶ್ನೆಯನ್ನು ಓದಿ, ನಿಮ್ಮ ಉತ್ತರವನ್ನು ರೆಕಾರ್ಡ್ ಮಾಡಿ",
  PRACTICE_READ_QUESTION_SELECT_ANSWER:
    "ಪ್ರಶ್ನೆಯನ್ನು ಓದಿ, ಸರಿಯಾದ ಉತ್ತರವನ್ನು ಆಯ್ಕೆಮಾಡಿ",
  PRACTICE_LOOK_PICTURE_SPEAK_ANSWER:
    "ಚಿತ್ರವನ್ನು ನೋಡಿ, ಕೆಳಗೆ ಕೊಟ್ಟಿರುವುದರಿಂದ ಸರಿಯಾದ ಉತ್ತರವನ್ನು ಹೇಳಿರಿ.",
  PRACTICE_LETTER_RECOGNITION: "ಅಕ್ಷರ ಗುರುತಿಸುವಿಕೆ",
  PRACTICE_MEMORY_CHALLENGE: "ನೆನಪಿನಶಕ್ತಿ ಸವಾಲು",
  PRACTICE_F3_APPLY_LETTER_SPEED: "ಅನ್ವಯಿಸಿ {step} - ಅಕ್ಷರ ವೇಗ",
  PRACTICE_F3_PRACTICE_LETTER_SPEED: "ಅಭ್ಯಾಸ {step} - ಅಕ್ಷರ ವೇಗ",
  PRACTICE_LETTER_LAUNCHER_TITLE: "ಅಕ್ಷರ ಲಾಂಚರ್",
  PRACTICE_LETTER_LAUNCHER_NOT_IMPLEMENTED:
    "ಅಕ್ಷರ ಲಾಂಚರ್ ಘಟಕವನ್ನು ಇನ್ನೂ ಕಾರ್ಯಗತಗೊಳಿಸಿಲ್ಲ.",
  PRACTICE_UNABLE_LOAD_CONTENT_RETRY:
    "ಲೋಡ್ ಮಾಡಲು ಆಗುತ್ತಿಲ್ಲ. ದಯವಿಟ್ಟು ಪುನಃ ಪ್ರಯತ್ನಿಸಿ.",
  PRACTICE_UNABLE_LOAD_F2_STEP_RETRY:
    "F2 ಫ್ಲೋ ಹಂತವನ್ನು ಲೋಡ್ ಮಾಡಲು ಆಗುತ್ತಿಲ್ಲ. ದಯವಿಟ್ಟು ಪುನಃ ಪ್ರಯತ್ನಿಸಿ..",
  PRACTICE_UNABLE_LOAD_F3_STEP_RETRY:
    "F3 ಫ್ಲೋ ಹಂತವನ್ನು ಲೋಡ್ ಮಾಡಲು ಆಗುತ್ತಿಲ್ಲ. ದಯವಿಟ್ಟು ಪುನಃ ಪ್ರಯತ್ನಿಸಿ.",
  PRACTICE_UNABLE_LOAD_F1_STEP_RETRY:
    "F1 ಫ್ಲೋ ಹಂತವನ್ನು ಲೋಡ್ ಮಾಡಲು ಆಗುತ್ತಿಲ್ಲ. ದಯವಿಟ್ಟು ಪುನಃ ಪ್ರಯತ್ನಿಸಿ.",
  PRACTICE_F3_LETTER_LAUNCHER_STUB_TITLE: "F3 ಫ್ಲೋ - ಲೆಟರ್ ಲಾಂಚರ್",
  PRACTICE_FORM_SENTENCE_SPEAK: "ಪದಗಳನ್ನು ಬಳಸಿ ವಾಕ್ಯ ರಚಿಸಿ ಮತ್ತು ಮಾತನಾಡಿ",
  PRACTICE_FILL_IN_THE_BLANK: "ಖಾಲಿ ಸ್ಥಳವನ್ನು ಭರ್ತಿ ಮಾಡಿ",

  // Practice status
  GAME_OVER: "ಗೇಮ್ ಓವರ್!",
  PLAY_AGAIN: "ಮತ್ತೆ ಆಡು",
  FEEDBACK_CORRECT: "🎉 ಸರಿಯಿದೆ!",
  FEEDBACK_WRONG: "😢 ಅಯ್ಯೋ! ತಪ್ಪು!",
  MEMORY_TIME_UP: "ಸಮಯ ಮುಗಿದಿದೆ!",
  MEMORY_CLICK_LETTERS: "ಕೆಳಗೆ ಅಕ್ಷರಗಳನ್ನು ಕ್ಲಿಕ್ ಮಾಡಿ...",
  MEMORY_REMOVE_LAST: "ಕೊನೆಯದನ್ನು ತೆಗೆದುಹಾಕಿ",
  MEMORY_CHECK_SEQUENCE: "ಅನುಕ್ರಮವನ್ನು ಪರಿಶೀಲಿಸಿ",
  MEMORY_CHECK: "ಪರಿಶೀಲಿಸಿ",
  SUCCESS_NEXT_LEVEL: "ಮುಂದಿನ ಹಂತ",
  SUCCESS_PERFECT: "ಅದ್ಭುತ! ನೀವು ಎಲ್ಲವನ್ನೂ ಸರಿಯಾಗಿ ಮಾಡಿದ್ದೀರಿ!",
  SUCCESS_EXCELLENT: "🎯 ಅತ್ಯುತ್ತಮ ಕೆಲಸ! ನೀವು ಬಹುತೇಕ ಪರ್ಫೆಕ್ಟ್!",
  SUCCESS_GREAT: "👏 ಭಲೇ! ನೀವು ತುಂಬಾ ಚೆನ್ನಾಗಿ ಕಲಿಯುತ್ತಿದ್ದೀರಿ!",
  SUCCESS_GOOD: "💪 ಒಳ್ಳೆಯ ಪ್ರಯತ್ನ! ಅಭ್ಯಾಸ ಮುಂದುವರೆಸಿ!",
  SUCCESS_NICE_TRY: "🌱 ನೈಸ್ ಟ್ರೈ! ಪ್ರತಿ ಪ್ರಯತ್ನವೂ ನಿಮ್ಮನ್ನು ಬಲಗೊಳಿಸುತ್ತದೆ!",
  SUCCESS_MASCOT_MESSAGE:
    "ನಾನು ನಿಮ್ಮ ಬಗ್ಗೆ ತುಂಬಾ ಹೆಮ್ಮೆಪಡುತ್ತೇನೆ! ಹೀಗೆ ಮುಂದುವರಿಯಿರಿ!",
  FUEL_LABEL: "ಇಂಧನ:",
  FUEL_READY_TO_LAUNCH: "ಉಡ್ಡಯನಕ್ಕೆ ಸಿದ್ಧ!",
  PRACTICE_MESSAGE_STEP_COMPLETE:
    "ನೀವು {stepName} ಅನ್ನು ಯಶಸ್ವಿಯಾಗಿ ಪೂರ್ಣಗೊಳಿಸಿದ್ದೀರಿ",

  // Assessment
  ASSESSMENT_SYLLABLE: "ಗುಣಿತಾಕ್ಷರ",
  LETTER_TRAIN_LETTER: "ಅಕ್ಷರ",
  LETTER_TRAIN_VOWEL: "ಸ್ವರ",
  LETTER_TRAIN_CONSONANT: "ವ್ಯಂಜನ",
  ASSESSMENT_TEST_LANGUAGE_SKILLS: "ನಿಮ್ಮ ಭಾಷಾ ಕೌಶಲ್ಯವನ್ನು ಪರೀಕ್ಷಿಸೋಣ",
  ASSESSMENT_GOOD_LANGUAGE_SKILLS: "ನಿಮಗೆ ಉತ್ತಮ ಭಾಷಾ ಕೌಶಲ್ಯವಿದೆ",
  ASSESSMENT_TAKE_COMPLETE_LEVEL:
    "ಹಂತವನ್ನು {level}  ಪೂರ್ಣಗೊಳಿಸಲು ಮೌಲ್ಯಮಾಪನ ತೆಗೆದುಕೊಳ್ಳಿ.",
  ASSESSMENT_TAKE_DISCOVER_LEVEL:
    "ನಿಮ್ಮ ಮಟ್ಟವನ್ನು ಕಂಡುಹಿಡಿಯಲು ಮೌಲ್ಯಮಾಪನ ತೆಗೆದುಕೊಳ್ಳಿ",
  ASSESSMENT_START_F1: "F1 ಪ್ರಾರಂಭಿಸಿ",
  ASSESSMENT_START_F2: "F2  ಪ್ರಾರಂಭಿಸಿ",
  ASSESSMENT_START_F3: "F3  ಪ್ರಾರಂಭಿಸಿ",
  ASSESSMENT_LEARN_LETTERS: "ಅಕ್ಷರಗಳನ್ನು ಕಲಿಯಿರಿ",
  ASSESSMENT_START_LEVEL: "ಹಂತವನ್ನು {level} ಆರಂಭಿಸಿ",
  ASSESSMENT_WORDS_LEARNT: "ಕಲಿತ ಪದಗಳು",
  ASSESSMENT_WORDS_PER_MINUTE: "ಪ್ರತಿ ನಿಮಿಷಕ್ಕೆ ಪದಗಳು",
  ASSESSMENT_SELECT_LANGUAGE: "ಭಾಷೆಯನ್ನು ಆರಿಸಿ",
  ASSESSMENT_LANGUAGE_SHORT: "ಭಾಷೆ",
  ASSESSMENT_AUDIO_TEST: "ಆಡಿಯೋ ಪರೀಕ್ಷೆ",
  ASSESSMENT_ALPHABET_CHART: "ಅಕ್ಷರ ಚಾರ್ಟ್",
  ASSESSMENT_LOGOUT: "ಲಾಗ್ಔಟ್",
  ASSESSMENT_GO_HOME_CHANGE_LANGUAGE: "ಭಾಷೆಯನ್ನು ಬದಲಾಯಿಸಲು ಹೋಂ ಸ್ಕ್ರೀನಿಗೆ ಹೋಗಿ",
  ASSESSMENT_DIALOG_OOPS: "ಅಯ್ಯೋ...",
  ASSESSMENT_DIALOG_CONTINUE: "ಮುಂದುವರಿಸಿ",
  ASSESSMENT_LANG_CONFIRM: "ದೃಢೀಕರಿಸಿ",
  ASSESSMENT_COMPLETE_ASSESSMENT:
    "ನೀವು {count}ನೇ ಮೌಲ್ಯಮಾಪನವನ್ನು ಯಶಸ್ವಿಯಾಗಿ ಪೂರ್ಣಗೊಳಿಸಿದ್ದೀರಿ",
  ASSESSMENT_END_LEVEL_COMPLETED:
    "ನೀವು Level {level} ಅನ್ನು ಯಶಸ್ವಿಯಾಗಿ ಪೂರ್ಣಗೊಳಿಸಿದ್ದೀರಿ",
  ASSESSMENT_END_MORE_PRACTICE: "ಸ್ವಲ್ಪ ಹೆಚ್ಚು ಅಭ್ಯಾಸ ಅಗತ್ಯ",
  ASSESSMENT_END_RATE_EXPERIENCE: "Level {level} ಗೆ ನಿಮ್ಮ ಅನುಭವವನ್ನು ರೇಟ್ ಮಾಡಿ",
  ASSESSMENT_START_ASSESSMENT: "ಮೌಲ್ಯಮಾಪನ ಪ್ರಾರಂಭಿಸಿ",

  // Discovery
  DISCOVER_END_GOOD_SKILLS_MESSAGE:
    "ನೀವು ಉತ್ತಮ ಭಾಷಾ ಕೌಶಲ್ಯ ಹೊಂದಿದ್ದೀರಿ. ನೀವು ಹಂತ {level} ನಿಂದ ಪ್ರಾರಂಭಿಸಬಹುದು. ಕಲಿಕೆಯ ಪ್ರಯಾಣ ಆರಂಭವಾಗಲಿ!",

  // Accessibility

  // Loading/Error states
  LOADING_OVERLAY_ARIA: "ಲೋಡ್ ಆಗುತ್ತಿದೆ, ದಯವಿಟ್ಟು ನಿರೀಕ್ಷಿಸಿ",
  LOADING_FETCHING_DETAILS:
    "ದಯವಿಟ್ಟು ನಿರೀಕ್ಷಿಸಿ, ನಾವು ನಿಮಗಾಗಿ ವಿವರಗಳನ್ನು ತರುತ್ತಿದ್ದೇವೆ…",
  ASSESSMENT_NO_USERNAME_QUERY_PARAM:
    "ದಯವಿಟ್ಟು query param ನಲ್ಲಿ username ಸೇರಿಸಿ",
  ASSESSMENT_ALPHABET_CHART_TOOLTIP_TITLE: "📚 ವರ್ಣಮಾಲೆ ಚಾರ್ಟ್‌",
  ASSESSMENT_ALPHABET_CHART_TOOLTIP_DESC:
    "ನಿಮಗೆ ಅಕ್ಷರಗಳು ಅಥವಾ ಗುಣಿತಾಕ್ಷರಗಳನ್ನು ನೆನಪಿಸಿಕೊಳ್ಳಲು ಸಹಾಯ ಬೇಕಾದರೆ, ಇಲ್ಲಿರುವ ಚಾರ್ಟ್‌ ನೋಡಿ.",
  ERROR_SPEECH_RECOGNITION_BROWSER_UNSUPPORTED:
    "ನಿಮ್ಮ ಬ್ರೌಸರ್‌ನಲ್ಲಿ ಮಾತು ಗುರುತಿಸುವಿಕೆಯನ್ನು ಬೆಂಬಲಿಸುವುದಿಲ್ಲ.",

  // Voice prompts
  VOICE_COULD_NOT_HEAR_AGAIN:
    "ಕ್ಷಮಿಸಿ ನನಗೆ ಧ್ವನಿ ಕೇಳಲು ಸಾಧ್ಯವಾಗಲಿಲ್ಲ. ದಯವಿಟ್ಟು ನೀವು ಪುನಃ ಮಾತನಾಡಬಹುದೇ?",
  VOICE_PROMPT_SPEAK_APPROPRIATELY: "ದಯವಿಟ್ಟು ಸರಿಯಾಗಿ  ಮಾತನಾಡಿ.",
  VOICE_PROMPT_SPEAK_PROPERLY: "ದಯವಿಟ್ಟು ಸ್ಪಷ್ಟವಾಗಿ ಮಾತನಾಡಿ.",

  // Main layout / game results
  MAIN_LAYOUT_READY_CHALLENGE: "ಸವಾಲಿಗೆ ಸಿದ್ಧರಿದ್ದೀರಾ?",
  MAIN_LAYOUT_GAMEOVER_GOOD_TRY_SPEED: "ಉತ್ತಮ ಪ್ರಯತ್ನ! ಹೆಚ್ಚಿನ ವೇಗ ಬೇಕು.",
  MAIN_LAYOUT_NEXT_LESSON: "ಮುಂದಿನ ಪಾಠ",
  MAIN_LAYOUT_OOPS_SLOW: "ಅಯ್ಯೋ, ಸ್ವಲ್ಪ ನಿಧಾನ!",
  MAIN_LAYOUT_PRACTICE_MORE: "ಇನ್ನಷ್ಟು ಅಭ್ಯಾಸ ಮಾಡೋಣ",
  A11Y_PAUSE_AUDIO: "ಆಡಿಯೋ ನಿಲ್ಲಿಸಿ",
  A11Y_PLAY_AUDIO: "ಆಡಿಯೋ ಪ್ಲೇ ಮಾಡಿ",
  MAIN_LAYOUT_START_GAME: "ಆಟವನ್ನು ಪ್ರಾರಂಭಿಸಿ ➜",
  MAIN_LAYOUT_PRACTICE_ARROW: "ಅಭ್ಯಾಸ ➜",
  MAIN_LAYOUT_GAMEOVER_NEED_POINTS:
    "ಓಹ್! ನೀವು ಪಾಸ್ ಮಾಡಲು {points} ಅಂಕಗಳು ಬೇಕಾಗುತ್ತವೆ. ನೀವು ಬಹುತೇಕ ಮುಗಿಸಿದ್ದೀರಿ, ಪುನಃ ಪ್ರಯತ್ನಿಸಿ!",

  // TOWRE flow
  TOWRE_MSG_LOOK_AT_WORDS:
    "ಪದಗಳನ್ನು ನೋಡಿ.ನೀವು ಅವುಗಳನ್ನು ಶೀಘ್ರದಲ್ಲೇ ಓದುತ್ತೀರಿ — ಎಡದಿಂದ ಬಲಕ್ಕೆ, ಮೇಲಿಂದ ಕೆಳಗೆ",
  TOWRE_MSG_NEXT_WORDS_SHORT: "ನಿಮ್ಮ ಮುಂದಿನ ಪದಗಳು ಬರುತ್ತಿವೆ!",
  TOWRE_MSG_NEXT_WORDS_GREAT: "ತುಂಬಾ ಚೆನ್ನಾಗಿದೆ! ನಿಮ್ಮ ಮುಂದಿನ ಪದಗಳು ಬರುತ್ತಿವೆ.",
  TOWRE_MSG_NEXT_SET_HINT:
    "ಕೆಳಗಿನ ಬಟನ್ ಕ್ಲಿಕ್ ಮಾಡಿದಾಗ ನೀವು ಪದಗಳ ಮುಂದಿನ ಸೆಟ್‌ಗೆ ಹೋಗುತ್ತೀರಿ.",
  TOWRE_MSG_SKIP_WORD_HINT:
    "ಒಂದು ಪದ ಹೇಳಲು ಸಾಧ್ಯವಾಗದಿದ್ದರೆ, ಮುಂದಿನ ಪದಕ್ಕೆ ಹೋಗಬಹುದು.",
  TOWRE_MSG_READY_SECONDS:
    "ನೀವು ಸಿದ್ಧರಾಗಿದ್ದೀರಾ? ನಿಮಗೆ 45 ಸೆಕೆಂಡ್‌ಗಳು ಇರುತ್ತವೆ.",
  TOWRE_MSG_READY_SECONDS_ICON:
    "ನೀವು ಸಿದ್ಧರಾಗಿದ್ದೀರಾ?⏱️ ನಿಮಗೆ 45 ಸೆಕೆಂಡ್‌ಗಳು ಇರುತ್ತವೆ.",
  TOWRE_MSG_READ_WORDS:
    "ಪದಗಳನ್ನು ಜೋರಾಗಿ ಓದಿ!ಒಂದೊಂದಾಗಿ!ಮೇಲಿನ ಎಡಭಾಗದಿಂದ ಪ್ರಾರಂಭಿಸಿ",
  TOWRE_BROWSER_SPEECH_WARNING:
    "ನಿಮ್ಮ ಬ್ರೌಸರ್ ಸ್ಪೀಚ್ ರೆಕಗ್ನಿಷನ್ ಅನ್ನು ಬೆಂಬಲಿಸದಿರಬಹುದು. ಉತ್ತಮ ಅನುಭವಕ್ಕಾಗಿ Google Chrome ಅಥವಾ Microsoft Edge ಬಳಸಿ.",
  TOWRE_MIC_ACCESS_REQUIRED:
    "ಸ್ಪೀಚ್ ರೆಕಗ್ನಿಷನ್‌ಗಾಗಿ ಮೈಕ್ರೋಫೋನ್ ಅಗತ್ಯ. ದಯವಿಟ್ಟು ಮೈಕ್ರೋಫೋನ್ ಅನುಮತಿ ನೀಡಿ ಮತ್ತೆ ಪ್ರಯತ್ನಿಸಿ.",
  TOWRE_SCREEN_TITLE: "ಟೌರ್ ಫ್ಲೋ",
  TOWRE_RESULT_WELL_DONE: "ತುಂಬಾ ಚೆನ್ನಾಗಿದೆ!",
  TOWRE_RESULT_READING_FASTER: "ನೀವು ವೇಗವಾಗಿ ಓದುತ್ತಿದ್ದೀರಿ.",
  TOWRE_SCORE_UNAVAILABLE_WARNING:
    "⚠️ ನಿಮ್ಮ ಬ್ರೌಸರ್ ಸ್ಪೀಚ್ ರೆಕಗ್ನಿಷನ್ ಅನ್ನು ಬೆಂಬಲಿಸುವುದಿಲ್ಲ, ಆದ್ದರಿಂದ ಪದಗಳನ್ನು ಸ್ವಯಂಚಾಲಿತವಾಗಿ ಸ್ಕೋರ್ ಮಾಡಲಾಗಲಿಲ್ಲ. ನಿಖರ ಫಲಿತಾಂಶಕ್ಕಾಗಿ Google Chrome ಅಥವಾ Microsoft Edge ಬಳಸಿ.",
  TOWRE_RESULT_WORDS_PER_MINUTE: "ನಿಮಿಷಕ್ಕೆ ಪದಗಳು",
  TOWRE_RESULT_NEW_WORDS_LEARNT: "ಹೊಸದಾಗಿ ಕಲಿತ ಪದಗಳು",
  TOWRE_RESULT_TOTAL_WORDS_LEARNT: "ಒಟ್ಟು ಕಲಿತ ಪದಗಳು",
  TOWRE_COUNTDOWN_READY: "ಪದಗಳನ್ನು ಓದಲು ಸಿದ್ಧರಾಗಿ!",
  TOWRE_COUNTDOWN_INSTRUCTION:
    "45 ಸೆಕೆಂಡ್‌ಗಳಲ್ಲಿ ಸಾಧ್ಯವಾದಷ್ಟು ವೇಗವಾಗಿ ಪದಗಳನ್ನು ಜೋರಾಗಿ ಓದಿ.",
  TOWRE_COUNTDOWN_STARTS_IN: "ಪ್ರಾರಂಭವಾಗುತ್ತದೆ",
  TOWRE_COMPLETED_TITLE: "ಚಟುವಟಿಕೆ ಪೂರ್ಣಗೊಂಡಿತು!",
  TOWRE_COMPLETED_SUBTITLE: "ನೀವು ಎಲ್ಲಾ ಪದ ಸೆಟ್‌ಗಳನ್ನು ನೋಡಿದ್ದೀರಿ.",
  TOWRE_VIEW_RESULTS: "ಫಲಿತಾಂಶ ನೋಡಿ",
  TOWRE_ATTEMPTED_WORDS: "ಪ್ರಯತ್ನಿಸಿದ ಪದಗಳು",
  TOWRE_CORRECT_WORDS: "ಸರಿಯಾದ ಪದಗಳು",
  TOWRE_UNATTEMPTED_WORDS: "ಪ್ರಯತ್ನಿಸದ ಪದಗಳು",
  TOWRE_RECORDING: "ರೆಕಾರ್ಡಿಂಗ್",

  // Misc components
  CONTENT_TEXT_MISSING: "ಪಠ್ಯ ಕಾಣುತ್ತಿಲ್ಲ",
  ASK_MORE_TITLE: "ಇನ್ನಷ್ಟು ಕೇಳಿ!",

  // Word wall
  WORD_WALL_TITLE: "{name} ಅವರ ವರ್ಡ್ ವಾಲ್",
  WORD_WALL_YOU_DID_IT: "ನೀವು ಮಾಡಿ ಮುಗಿಸಿದಿರಿ!",
  WORD_WALL_WORDS_YOU_KNOW: "ನಿಮಗೆ ತಿಳಿದಿರುವ ಪದಗಳು",
  WORD_WALL_WORDS_TRY_AGAIN: "ಪುನಃ ಪ್ರಯತ್ನಿಸಬೇಕಾದ ಪದಗಳು!",
  WORD_WALL_OPEN_GIFTS: "ಉಡುಗೊರೆಗಳನ್ನು ತೆರೆಯೋಣ",
  WORD_WALL_CLICK_BUBBLE: "ಬಬಲ್ ಕ್ಲಿಕ್ ಮಾಡಿ",
  WORD_WALL_REVIEW_THIS_IS_CALLED: "ಇದನ್ನು... ಎಂದು ಕರೆಯಲಾಗುತ್ತದೆ",
  WORD_WALL_CAN_YOU_FIND: "ನೀವು ಕಂಡುಹಿಡಿಯಬಲ್ಲಿರಾ?",
  WORD_WALL_LISTEN_ENGLISH: "ಇಂಗ್ಲಿಷ್",

  // Fluency

  // Bingo card
  BINGO_WELL_DONE: "ಚೆನ್ನಾಗಿ ಮಾಡಿದ್ದೀರಿ!",
  BINGO_WORDS_FOUND: "ನೀವು {count} ಪದಗಳನ್ನು ಕಂಡುಕೊಂಡಿದ್ದೀರಿ",
  SESSION_EXPIRED_DEFAULT:
    "ನಿಮ್ಮ ಸೆಶನ್ ಮುಗಿದಿದೆ. ದಯವಿಟ್ಟು ಮತ್ತೆ ಸೈನ್ ಇನ್ ಮಾಡಿ.",
};

export default kn;
