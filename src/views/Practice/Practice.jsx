    ...
    // The LetterTrain check below will handle F1 Learn steps
      ...
      if (isF2FlowActive) {
        ...
        if (f2StepType === "L") {
          ...
          let customLettersForF2 = currentGetContentForF2?.customLetters; // Extract customLetters from F2 config
          // If customLetters contains words/syllables, extract individual letters for Letter Hunt
          if (
            customLettersForF2 &&
            Array.isArray(customLettersForF2) &&
            customLettersForF2.length > 0
          ) {
            // Check if first item is a word (length > 1) or a single letter
            const isWords = customLettersForF2.some(
              (item) => item && item.length > 1
            );
            if (isWords) {
              // Extract unique individual letters from words
              const lettersSet = new Set();
              customLettersForF2.forEach((word) => {
              ...
              customLettersForF2 = Array.from(lettersSet);
              console.log(
                "LetterHunt - Converted word-based customLetters to individual letters:",
                customLettersForF2
              );
            }
          }
      ...
      // If mechanism is letterHunt but F1 flow step type is "L", something is wrong - don't render
        ...
        if (f1StepType === "L") {
          ...
          let customLettersForF1 = currentGetContentForF1?.customLetters; // Extract customLetters from F1 config
          // If customLetters contains words/syllables, extract individual letters for Letter Hunt
          if (
            customLettersForF1 &&
            Array.isArray(customLettersForF1) &&
            customLettersForF1.length > 0
          ) {
            // Check if first item is a word (length > 1) or a single letter
            const isWords = customLettersForF1.some(
              (item) => item && item.length > 1
            );
            if (isWords) {
              // Extract unique individual letters from words
              const lettersSet = new Set();
              customLettersForF1.forEach((word) => {
                if (word && typeof word === "string") {
                  word.split("").forEach((char) => {
                    if (char.match(/[a-zA-Z]/)) {
                      lettersSet.add(char.toLowerCase());
              ...
              customLettersForF1 = Array.from(lettersSet);
              console.log(
                "LetterHunt - Converted word-based customLetters to individual letters:",
                customLettersForF1
              );
